...
const ImageMore = () => {
    return (
        <>
        {data?.map((item) => (
            <Link to={`/${item.id}`} key={item.id} className="md:max-w-lg xl:max-w-xl sm:flex flex-col-reverse justify-end xl:items-center md:pl-5 md:border-l cursor-pointer">
            
            <div>
                <h2 className="font-serif text-black text-lg hover:text-gray-600">{item.title}</h2>
                <p className="font-sans text-gray-500">{item.short_description}</p>
                <small className="uppercase text-xs text-gray-600">{item.read_time}</small>
            </div>
            <div className=" pt-5 sm:pt-0">
                <img src={item.image.src} alt={item.image.alt}  />
                <small className="flex justify-end uppercase text-[10px] text-gray-600 pt-1">{item.image.caption}</small>
            </div>
        </Link>
        ))}
